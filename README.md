...
Set Up the MySQL Database 🗄️

Connect to your MySQL server using a client (e.g., MySQL Workbench, phpMyAdmin, or command line).

Create the health_data database and patient_data table with the following SQL commands, including optional parameters for performance and compatibility:
...
CHARACTER SET utf8mb4: Supports a wide range of Unicode characters (e.g., Turkish characters).
COLLATE utf8mb4_unicode_ci: Ensures case-insensitive and accent-insensitive comparisons.
ENGINE=InnoDB: Provides transactional safety and better performance for frequent updates.
...
Update config.py with your MySQL credentials and Flask settings, ensuring DB_NAME is set to "health_data". Optionally, add connection parameters in your script (e.g., connection_timeout=300 for 5-minute timeout).
...
MySQL Veritabanını Kurun 🗄️

MySQL sunucusuna bir istemciyle bağlanın (örn. MySQL Workbench, phpMyAdmin veya komut satırı).

health_data veritabanını ve patient_data tablosunu, performans ve uyumluluk için ek parametrelerle oluşturmak için şu SQL komutlarını çalıştırın:
CREATE DATABASE IF NOT EXISTS health_data
CHARACTER SET utf8mb4
COLLATE utf8mb4_unicode_ci;
USE health_data;
...
CHARACTER SET utf8mb4: Türkçe karakterler dahil geniş bir Unicode desteği sağlar.
COLLATE utf8mb4_unicode_ci: Büyük/küçük harf ve aksanlara duyarsız karşılaştırmalar sağlar.
ENGINE=InnoDB: İşlem güvenliği ve sık güncellemelerde daha iyi performans sunar.
...
config.py dosyasını veritabanı kimlik bilgileri ve Flask ayarlarıyla güncelleyin, DB_NAME değerinin "health_data" olduğundan emin olun. İsteğe bağlı olarak, betiğinizde bağlantı parametreleri ekleyebilirsiniz (örneğin, connection_timeout=300 için 5 dakikalık zaman aşımı).



Ollama'yı Yükleyin 🖥️

Ollama'yı resmi web sitesinden indirip yükleyin.
